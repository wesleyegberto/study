 ...
 * 
 *         Computador Simpletron, e sua linguagem Simpletron Machine Language
 *         (SML)
 */
...
public class SimpletronMachineLanguage {
	...
	// Onde os dois primeiro digitos são os comandos, e os dois último é a
	// posição na memória
	...
	int instructionCounter; // monitora a posição da memória que contém a
							// instrução sendo utilizada
	...
	final int READ = 10; // Lê uma palavra do teclado para uma posição
							// específica da memória
	final int WRITE = 11; // Escreve na tela uma palavra de uma posição
							// específica da memória

	// Operações de Carregamento e Armazenamento
	final int LOAD = 20; // Carrega uma palavra de uma posição específica na
							// memória para o acumulador
	final int STORE = 21; // Armazena uma palavra do acumulador para uma posição
							// específica da memória

	// Operações Aritméticas, deixa o resultado no acumulador
	final int ADD = 30; // Adiciona uma palavra de uma posição específica na
						// memória à palavra no acumulador
	final int SUBTRACT = 31; // Subtrai uma palavra de uma posição específica na
								// memória da palavra no acumulador
	final int DIVIDE = 32; // Divide uma palavra de uma posição específica na
							// memória da palavra no acumulador
	final int MULTIPLY = 33; // Multiplica uma palavra de uma posição específica
								// na memória pela palavra no acumulador

	// Operações de Transferência de Controle
	final int BRANCH = 40; // Desvia para uma posição específica na memória
	final int BRANCHNEG = 41; // Desvia para uma posição específica na memória
								// se o acumulador for negativo
	final int BRANCHZERO = 42; // Desvia para uma posição específica na memória
	...
	public void start() {
		while(aux != 44 && aux != HALT && instructionCounter < 100) {
			System.out.printf("\n%02d ? ", instructionCounter);
			aux = readCmd();
			if(aux == 0000) // Será 0000 caso a posição na memória seja usada
							// para uma variável
			{
				instructionCounter++;
				continue;
			} else if(aux == 44 && aux != HALT && instructionCounter < 100)
				break;
	...
	public void debbug() {
		...
		while(instructionCounter < 100 && operationCode != HALT) {
			...
			if(instructionRegister == 0000) // Será 0000 caso a posição na
											// memória seja usada para uma
											// variável
				continue;
	...
	public void operationCodeView(int operCode, int oper) {
		switch(operCode) {
			case READ:
				...
				break;
			case WRITE:
				System.out.println(memory[oper]);
				break;
			case LOAD:
				accumulator = memory[oper];
				break;
			case STORE:
				memory[oper] = accumulator;
				break;
			case ADD:
				accumulator += memory[oper];
				break;
	...
	/**
	 * Exibe o nome e o conteúdo de cada resgistrador do Simpletron e sua
	 * memória
	 */
	@SuppressWarnings("boxing")
	public void dump() {
